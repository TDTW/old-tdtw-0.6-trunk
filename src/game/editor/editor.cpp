...
void CEditor::DoToolbar(CUIRect ToolBar)
{
	...
	if(DoButton_Ex(&s_ZoomNormalButton, Localize("1:1"), 0, &Button, 0, Localize("[NumPad*] Zoom to normal and remove editor offset"), 0) || Input()->KeyDown(KEY_KP_MULTIPLY))
	{
		m_EditorOffsetX = 0;
		m_EditorOffsetY = 0;
		m_ZoomLevel = 100;
	...
	if(DoButton_Ex(&s_AnimFasterButton, Localize("A+"), 0, &Button, 0, Localize("Increase animation speed"), CUI::CORNER_L))
		m_AnimateSpeed += 0.5f;

	TB_Top.VSplitLeft(30.0f, &Button, &TB_Top);
	static int s_AnimNormalButton = 0;
	if(DoButton_Ex(&s_AnimNormalButton, Localize("1"), 0, &Button, 0, Localize("Normal animation speed"), 0))
		m_AnimateSpeed = 1.0f;

	TB_Top.VSplitLeft(30.0f, &Button, &TB_Top);
	static int s_AnimSlowerButton = 0;
	if(DoButton_Ex(&s_AnimSlowerButton, Localize("A-"), 0, &Button, 0, Localize("Decrease animation speed"), CUI::CORNER_R))
	{
	...
	// brush manipulation
	{
		...
		if(DoButton_Ex(&s_FlipXButton, Localize("X/X"), Enabled, &Button, 0, Localize("[N] Flip brush horizontal"), CUI::CORNER_L) || Input()->KeyDown('n'))
		{
		...
		if(DoButton_Ex(&s_FlipyButton, Localize("Y/Y"), Enabled, &Button, 0, Localize("[M] Flip brush vertical"), CUI::CORNER_R) || Input()->KeyDown('m'))
		{
			for(int i = 0; i < m_Brush.m_lLayers.size(); i++)
			...
			if(UI()->ActiveItem() == s_pEditorID)
			{
				...
				else if(s_Operation == OP_BRUSH_GRAB)
				{
					if(!UI()->MouseButton(0))
					{
						// grab brush
						char aBuf[256];
						str_format(aBuf, sizeof(aBuf),Localize("grabbing %f %f %f %f"), r.x, r.y, r.w, r.h);
	...
	// render layers
	{
		for(int g = 0; g < m_Map.m_lGroups.size(); g++)
		{
			...
			if(LayerCur >= LayerStartAt)
			{
				...
				if(DoButton_Ex(&m_Map.m_lGroups[g]->m_Visible, m_Map.m_lGroups[g]->m_Visible?"V":"H", m_Map.m_lGroups[g]->m_Collapse ? 1 : 0, &VisibleToggle, 0, Localize("Toggle group visibility"), CUI::CORNER_L))
				...
				if(int Result = DoButton_Ex(&m_Map.m_lGroups[g], aBuf, g==m_SelectedGroup, &Slot,
					BUTTON_CONTEXT, m_Map.m_lGroups[g]->m_Collapse ? Localize("Select group. Double click to expand.") : Localize("Select group. Double click to collapse."), 0, FontSize))
				{
					m_SelectedGroup = g;
					m_SelectedLayer = 0;
			...
			for(int i = 0; i < m_Map.m_lGroups[g]->m_lLayers.size(); i++)
			{
				...
				if(int Result = DoButton_Ex(m_Map.m_lGroups[g]->m_lLayers[i], aBuf, g==m_SelectedGroup&&i==m_SelectedLayer, &Button,
					BUTTON_CONTEXT, Localize("Select layer."), 0, FontSize))
				{
					m_SelectedLayer = i;
					m_SelectedGroup = g;
...
void CEditor::RenderFileDialog()
{
	...
	if(DoButton_Editor(&s_OkButton, IsDir ? Localize("Open") : m_pFileDialogButtonText, 0, &Button, 0, 0) || m_aFileDialogActivate)
	{
		...
		else // file
		{
			...
			if(!str_comp(m_pFileDialogButtonText, Localize("Save")))
			{
...
void CEditorMap::MakeGameGroup(CLayerGroup *pGroup)
{
	...
	str_copy(m_pGameGroup->m_aName, Localize("Game"), sizeof(m_pGameGroup->m_aName));
}
