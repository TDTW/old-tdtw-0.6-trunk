...
{
	Console()->Print(IConsole::OUTPUT_LEVEL_DEBUG, Localize("editor"), Localize("invoked"));
...
int CEditor::PopupQuad(CEditor *pEditor, CUIRect View)
{
	...
	if(pEditor->DoButton_Editor(&s_AlignButton, Localize("Align"), 0, &Button, 0, Localize("Aligns coordinates of the quad points")))
	{
		for(int k = 1; k < 4; k++)
		{
	...
	CProperty aProps[] = {
		{Localize("Pos X"), pQuad->m_aPoints[4].x/1000, PROPTYPE_INT_SCROLL, -1000000, 1000000},
		{Localize("Pos Y"), pQuad->m_aPoints[4].y/1000, PROPTYPE_INT_SCROLL, -1000000, 1000000},
...
int CEditor::PopupPoint(CEditor *pEditor, CUIRect View)
{
	...
	CProperty aProps[] = {
		{Localize("Pos X"), x, PROPTYPE_INT_SCROLL, -1000000, 1000000},
		{Localize("Pos Y"), y, PROPTYPE_INT_SCROLL, -1000000, 1000000},
...
int CEditor::PopupNewFolder(CEditor *pEditor, CUIRect View)
{
	...
	pEditor->UI()->DoLabel(&Label, Localize("Create new folder"), 20.0f, 0);
	...
	if(pEditor->m_FileDialogErrString[0] == 0)
	{
		...
		View.HSplitBottom(20.0f, &View, &Label);
		pEditor->UI()->DoLabel(&Label, Localize("Name:"), 10.0f, -1);

		// button bar
		ButtonBar.VSplitLeft(30.0f, 0, &ButtonBar);
		ButtonBar.VSplitLeft(110.0f, &Label, &ButtonBar);
		static int s_CreateButton = 0;
		if(pEditor->DoButton_Editor(&s_CreateButton, Localize("Create"), 0, &Label, 0, 0))
		{
			// create the folder
			if(*pEditor->m_FileDialogNewFolderName)
			{
				...
				if(pEditor->Storage()->CreateFolder(aBuf, IStorage::TYPE_SAVE))
				{
					...
					str_copy(pEditor->m_FileDialogErrString, Localize("Unable to create the folder"), sizeof(pEditor->m_FileDialogErrString));
			}
		}
		ButtonBar.VSplitRight(30.0f, &ButtonBar, 0);
		ButtonBar.VSplitRight(110.0f, &ButtonBar, &Label);
		static int s_AbortButton = 0;
		if(pEditor->DoButton_Editor(&s_AbortButton, Localize("Abort"), 0, &Label, 0, 0))
			return 1;
	}
	else
	{
		...
		pEditor->UI()->DoLabel(&Label, Localize("Error:"), 10.0f, -1);
		View.HSplitTop(20.0f, &Label, &View);
		pEditor->UI()->DoLabel(&Label, Localize("Unable to create the folder"), 10.0f, -1, View.w);

		// button
		ButtonBar.VMargin(ButtonBar.w/2.0f-55.0f, &ButtonBar);
		static int s_CreateButton = 0;
		if(pEditor->DoButton_Editor(&s_CreateButton, Localize("Ok"), 0, &ButtonBar, 0, 0))
			return 1;
	}
...
int CEditor::PopupEvent(CEditor *pEditor, CUIRect View)
{
	...
	if(pEditor->m_PopupEventType == POPEVENT_EXIT)
		pEditor->UI()->DoLabel(&Label, Localize("Exit the editor"), 20.0f, 0);
	else if(pEditor->m_PopupEventType == POPEVENT_LOAD)
		pEditor->UI()->DoLabel(&Label, Localize("Load map"), 20.0f, 0);
	else if(pEditor->m_PopupEventType == POPEVENT_NEW)
		pEditor->UI()->DoLabel(&Label, Localize("New map"), 20.0f, 0);
	else if(pEditor->m_PopupEventType == POPEVENT_SAVE)
		pEditor->UI()->DoLabel(&Label, Localize("Save map"), 20.0f, 0);
	...
	View.HSplitTop(20.0f, &Label, &View);
	if(pEditor->m_PopupEventType == POPEVENT_EXIT)
		pEditor->UI()->DoLabel(&Label, Localize("The map contains unsaved data, you might want to save it before you exit the editor.\nContinue anyway?"), 10.0f, -1, Label.w-10.0f);
	else if(pEditor->m_PopupEventType == POPEVENT_LOAD)
		pEditor->UI()->DoLabel(&Label, Localize("The map contains unsaved data, you might want to save it before you load a new map.\nContinue anyway?"), 10.0f, -1, Label.w-10.0f);
	else if(pEditor->m_PopupEventType == POPEVENT_NEW)
		pEditor->UI()->DoLabel(&Label, Localize("The map contains unsaved data, you might want to save it before you create a new map.\nContinue anyway?"), 10.0f, -1, Label.w-10.0f);
	else if(pEditor->m_PopupEventType == POPEVENT_SAVE)
		pEditor->UI()->DoLabel(&Label, Localize("The file already exists.\nDo you want to overwrite the map?"), 10.0f, -1);

	// button bar
	ButtonBar.VSplitLeft(30.0f, 0, &ButtonBar);
	ButtonBar.VSplitLeft(110.0f, &Label, &ButtonBar);
	static int s_OkButton = 0;
	if(pEditor->DoButton_Editor(&s_OkButton, Localize("Ok"), 0, &Label, 0, 0))
	{
		if(pEditor->m_PopupEventType == POPEVENT_EXIT)
...
{
	static const char *s_pButtonNames[] = { Localize("Clear"), Localize("Collision"), Localize("Death"), Localize("Unhookable") };
